...
public class ConfirmEmailInteractorImpl implements ConfirmEmailContract.Interactor {

    private ApiInterfaces apiService =
            ApiClient.getInstance().create(ApiInterfaces.class);
    private String TAG = "LOGGERR Confirm Email";

    @Override
    public void confirmEmail(final OnFinishedListener onFinishedListener, String email, String code) {
        Call<CommonResponse> sendEmailCall = apiService.confirmEmail(
                email,
                code
        );
